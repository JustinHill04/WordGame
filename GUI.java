...
public class GUI extends JFrame {
    ...
    private void setupMenuBar() {
        ...
        aboutAppItem.addActionListener(e -> 
            JOptionPane.showMessageDialog(this, 
                ...
                "About Application",
                "Image Attributions: Winner, Medal, Gold image by AxxLC / Confetti, Concert, People image by StockSnap",
                "Sound Attributions: 09569 trumpets ta da fanfare.wav by Robinhood76",
    ...
    animationLabel = new JLabel(new ImageIcon(getClass().getResource("/images/confetti.png")));
    animationLabel.setVisible(false);
    animationLabel.setOpaque(false);
    animationLabel.setBounds(100, -200, 300, 200); // starting offscreen position
    getLayeredPane().add(animationLabel, JLayeredPane.PALETTE_LAYER);
    ...
    // Reset position and show
        animationY = -200;
        animationLabel.setLocation(100, animationY);
        animationLabel.setVisible(true);

    // Timer moves the image downward
        animationTimer = new Timer(25, e -> {
        animationY += 8;
        animationLabel.setLocation(100, animationY);

        // Stop after it passes the center
        if (animationY > getHeight()) {
        ...
        new Thread(() -> {
            ...
            while (!gameOver) {
                for (Player p : players) {
                    ...
                    if (game.isWon()) {
                        ImageIcon icon = new ImageIcon(getClass().getResource("winner.jpg"));
